...
1. **Resolve every `TODO` placeholder in `pyproject.toml`.** `grep -n TODO pyproject.toml` to find them all: `[project] name`, `description`, `authors`; `packages = ["src/TODO"]` and `version-file = "src/TODO/_version.py"`; `[tool.coverage.paths] source`/`omit`; `[tool.ruff] exclude`; `[tool.ruff.lint.isort] known-first-party`; `pyrefly check src/TODO`; and the three `--cov=TODO` occurrences in the `unit-tests`/`acceptance-tests`/`integration-tests` scripts. Never create `src/snake_case({ProjectName})/_version.py` by hand — `hatch-vcs` generates it at build time and it's gitignored. **`compose.yaml` at the project root also carries `TODO` placeholders** — `grep -n TODO compose.yaml` to find them: the `app`, `eventstore`, `projections`, and `observability` services' `container_name`s, `OTEL_SERVICE_NAME`, and the three `POSTGRES_USER` occurrences (the two env vars and the `pg_isready` healthcheck command).
