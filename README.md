...
```bash
node .build-kit-node/code-export.mjs
```
...
- `.build-kit-node/.slices/<context>/config.json` — config scoped to the context
- `.build-kit-node/.slices/<context>/index.json` — slice index with status and folder mappings
- `.build-kit-node/.slices/<context>/<slice>/slice.json` — one file per slice
- `.build-kit-node/.slices/<context>/<slice>/screen-<id>.png` — slice screenshots (if present)
- `.build-kit-node/.slices/current_context.json` — pointer to the active context

Override the default port with an environment variable:

```bash
PORT=3002 node .build-kit-node/code-export.mjs
```
