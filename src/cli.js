...
const IGNORED_COPY_DIRS = ['node_modules'];
const KIT_NAME = 'build-kit-pyeventsourcing';

const program = new Command();

program
  .name('${KIT_NAME}')
  .description('${KIT_NAME} — real-time Claude agent that reacts to slice:changed events on an Eventmodelers board')
  .version('0.1.0');

program
  .command('install')
  .description('Install ${KIT_NAME} into the current directory')
  .action(async () => {
...
program
  .command('uninstall')
  .description('Remove ${KIT_NAME} files from current directory')
  .action(() => {
    const targets = [
